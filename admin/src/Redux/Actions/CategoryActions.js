...
export const ListCategory = () => async (dispatch, getState) => {
  try {
    ...
    const { data } = await request.get(`/categories?level=2`, config);
...
export const DeleteCategory = (i) => async (dispatch, getState) => {
  try {
    ...
    const { data } = await request.delete(`/categories/${i}`, config);
...
export const addCategory = (name, description) => async (dispatch, getState) => {
  try {
    ...
    const { data } = await request.post(`/categories`, { name, description }, config);
...
export const UpdateCurrentCategory = (idCategory, name, description) => async (dispatch, getState) => {
  try {
    ...
    const { data } = await request.put(`/categories`, { idCategory, name, description }, config);
